...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await syncSpicesBoard({ slot: "slot_1100" });
    return NextResponse.json({ ok: true, result, slot: "slot_1100" });
  } catch (error) {
    console.error("GET /api/cron/daily-sync-1100 failed", error);
